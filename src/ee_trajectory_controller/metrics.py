...
def computeMetrics(t: trajectory, targetPose: Pose, slidingWindowTime: np.float64 ):
    ...
    for eeRot, objRot in zip(eeRotations, objRotations):
        eeRotVec = findRotVec(eeRot)
        objRotVec = findRotVec(objRot)
    ...
    for eeRot, objRot in zip(eeRotations, objRotations):
        rotationErrorVec = errorRotVec(eeRot, objRot)
...
def plotMetrics(t: trajectory, targetPose: Pose, metrics):
    


    fig, axes = plt.subplots(2,3, figsize = (12, 10))
    
    #show x,y,z position against commanded
    ax = axes[0,0]
    ...
    #show x,y,z rotation vector against commanded
    ax = axes[0,1]
    ...
    #show x,y,z linear velocity against commanded
    ax = axes[0,2]
    ...
    ax.plot(metrics['time'], metrics['object_linear_velocities'][:,0], color = "yellow", label = "eelinearVelocity_X")
    ax.plot(metrics['time'], metrics['object_linear_velocities'][:,1], color = "purple", label = "eelinearVelocity_Y")
    ax.plot(metrics['time'], metrics['object_linear_velocities'][:,2], color = "orange", label = "eelinearVelocity_Z")
